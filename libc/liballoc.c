...
#define l_pageSize  4096			///< The size of an individual page. Set up in liballoc_init.
#define l_pageCount 16			///< The number of pages to request per chunk. Set up in liballoc_init.
...
static void liballoc_dump()
{
	...
	serial_printf( "liballoc: System memory allocated: %d bytes\n", l_allocated );
	serial_printf( "liballoc: Memory in used (malloc'ed): %d bytes\n", l_inuse );
	serial_printf( "liballoc: Warning count: %d\n", l_warningCount );
	serial_printf( "liballoc: Error count: %d\n", l_errorCount );
	serial_printf( "liballoc: Possible overruns: %d\n", l_possibleOverruns );

#ifdef DEBUG
		while ( maj != NULL )
		{
			serial_printf( "liballoc: %x: total = %d, used = %d\n",
						maj, 
						maj->size,
						maj->usage );
			...
			while ( min != NULL )
			{
				serial_printf( "liballoc:    %x: %d bytes\n",
							min, 
							min->size );
				min = min->next;
			}
...
static struct liballoc_major *allocate_new_page( unsigned int size )
{
	...
	struct liballoc_major *maj;
		...
		#ifdef DEBUG
		serial_printf( "liballoc: Resource allocated %x of %d pages (%d bytes) for %d size.\n", maj, st, maj->size, size );
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	#ifdef DEBUG
	serial_printf( "liballoc: %x PREFIX(malloc)( %d ): ", 
					__builtin_return_address(0),
					size );
	FLUSH();
	#endif
